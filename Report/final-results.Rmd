...
make_graph(only_blocks(sub_walk_all, 1:3), "learning.condition", "min_norm_distance", "exp_block_id") + 
  ylab("Average normalised trial distance") + xlab("Learning condition") + 
  scale_fill_manual(values=estimote_pal,"Block") + guides(fill=F)

make_graph(only_blocks(sub_walk_all, 1:3), "learning.condition", "errors", "exp_block_id") + 
  ylab("Average normalised trial distance") + xlab("Learning condition") + 
  scale_fill_manual(values=estimote_pal,"Block") + guides(fill=F)

ggplot(point_data_1, aes(x = learning.condition, y = abs_error, fill = learning.condition)) +
  stat_summary(fun.data=mean_cl_normal,position=position_dodge(0.95), geom="errorbar") + 
  stat_summary(fun.y=mean,position=position_dodge(width=0.95),geom="bar") +
  ylab("Average pointing error in degrees after learning phase") + xlab("Learning condition") + 
  scale_fill_manual(values = estimote_pal, "Learning condition")  + guides(fill=F)
...
walk_all_filtered <- rbind(sub_walk_all, sub_walk_all_2)
ggplot(only_blocks(walk_all_filtered, 6), aes(x = condition, y = min_norm_distance, fill = learning.condition)) +
  stat_summary(fun.data=mean_cl_normal,position=position_dodge(0.95), geom="errorbar") + 
  stat_summary(fun.y=mean,position=position_dodge(width=0.95),geom="bar") +
  ylab("Average path distance in the last block of the transfer phase") + xlab("Condition") + 
  scale_fill_manual(values = estimote_pal, "Condition")  + guides(fill=F)

ggplot(only_blocks(walk_all_filtered, 6), aes(x = condition, y = errors, fill = learning.condition)) +
  stat_summary(fun.data=mean_cl_normal,position=position_dodge(0.95), geom="errorbar") + 
  stat_summary(fun.y=mean,position=position_dodge(width=0.95),geom="bar") +
  ylab("Number of visiting errors in the last block of the transfer phase") + xlab("Condition") + 
  scale_fill_manual(values = estimote_pal, "Condition")  + guides(fill=F)
```
