...
all_participant_experiment <- overview %>% left_join(all_participant_experiment, by="Code")
...
df_wide_errors$block34[is.na(df_wide_errors$block34)] <- 0 #fixes case where person scores 0 in block 3 and 4 and so we have
...
We hypothesized that the degree of transfer to the real-world building would vary as a function of immersion. As a first test of this issue, we wished to determine whether all modalities resulted in some degree of transfer. Data for distance walked and number of door visitation errors were each entered into a 3 (Learning Condition: real, desktop, treadmill VR) × 2 (Block: block 1, block 4) mixed-model analysis of variance (ANOVA). For distance walked, there was a main effect for Learning Condition, `r apa_print(ez_fit_distance$aov)$full_result$learning_condition`. There was a also a main effect for Block, `r apa_print(ez_fit_distance$aov)$full_result$exp_block_id`. The Learning Condition by Block interaction effect did not reach significance `r apa_print(ez_fit_distance$aov)$full_result$learning_condition_exp_block_id`. Follow-up comparisons showed that for walked distance all three modalities showed an improvement from block 1 (first exposure to the modality) to block 4 (first exposure to the real building for immersed/impoverished conditions, fourth exposure for real-world). Descriptive and inferential statistics for these comparisons are shown in Table X and Table X+1 for distance and errors, respectively.

For visitation errors, there was no main effect for Learning Condition,`r apa_print(ez_fit_errors$aov)$full_result$learning_condition`. There was a main effect for Block, `r apa_print(ez_fit_errors$aov)$full_result$exp_block_id`. The Learning Condition by Block interaction effect did not reach significance, `r apa_print(ez_fit_errors$aov)$full_result$learning_condition_exp_block_id`. Follow-up comparisons showed that all three modalities showed an improvement in visitation errors from block 1 (first exposure to the modality) to block 4 (first exposure to the real building for immersed/impoverished conditions, fourth exposure for real-world). Descriptive and inferential statistics for these comparisons are shown in Table X+1.
...
3x2 repeated measures ANOVA revealed a main effect of block on distance performance `r apa_print(ez_fit_distance$aov)$full_result$exp_block_id`, suggesting that participants improved their paths in all three conditions. Although the effect of the learning modality was significant (`r apa_print(ez_fit_distance$aov)$full_result$learning_condition`), the interaction between block and modality was not significant, `r apa_print(ez_fit_distance$aov)$full_result$learning_condition_exp_block_id`, suggesting that learning rate did not differ as a function of modality. In contrast, for visitation errors,  we again found a main effect of block (`r apa_print(ez_fit_errors$aov)$full_result$exp_block_id`) adn modality (`r apa_print(ez_fit_errors$aov)$full_result$learning_condition`), but, importantly, a block X modality interaction effect, `r apa_print(ez_fit_errors$aov)$full_result$learning_condition_exp_block_id`. This finding suggested that visitation errors improved differently as a function of modality.

To investigate further, we conducted separate one-way ANOVAs to compare learning conditions for walked distance and visitation errors for the last block of learning (block 3). There were main effects of learning condition for both walked distance, `r apa_print(aov(min_norm_distance ~ learning.condition, only_blocks(sub_walk_all, 3)))$full_result`, and visitation errors, `r apa_print(aov(errors ~ learning.condition, only_blocks(sub_walk_all, 3)))$full_result`). Comparing the conditions closer, we find that there is a significant difference in visitations error performance between real world and desktop learning (`r t_test_effect(errors ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Real', 'Desktop'), 3))`) and real world nad treadmill VR learning (`r t_test_effect(errors ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Real', 'Treadmill VR'), 3))`), but no difference between Desktop and Treadmill learning (`r t_test_effect(errors ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Desktop', 'Treadmill VR'), 3))`). The situation is the same for distance comparisons. real world and desktop learning (`r t_test_effect(min_norm_distance ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Real', 'Desktop'), 3))`) and real world nad treadmill VR learning (`r t_test_effect(min_norm_distance ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Real', 'Treadmill VR'), 3))`), but no difference between Desktop and Treadmill learning (`r t_test_effect(min_norm_distance ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Desktop', 'Treadmill VR'), 3))`).

```{r}
point_data_1 <- sub_sop_all[sub_sop_all$phase == 1,]
aov_point_phase1 <- aov(abs_error ~ learning.condition, data = point_data_2)
```

We also observed significant differences between conditions in pointing performance at the end of the first phase, `r apa_print(aov_point_phase1)$full_result$learning_condition`.  Specifically, participants in the real-world learning group  (`r m_sd_report(only_conditions(point_data_1, "Real")$abs_error)`) performed significantly better compared to both the immersive VR group (`r m_sd_report(only_conditions(point_data_1, "Treadmill VR")$abs_error)`), `r t_test_effect(abs_error ~ learning.condition, only_conditions(point_data_1, c('Real', 'Treadmill VR')))`, and the desktop group (`r m_sd_report(only_conditions(point_data_1, "Desktop")$abs_error)`), `r t_test_effect(abs_error ~ learning.condition, only_conditions(point_data_1, c('Real', 'Desktop')))`, but no difference between VR and desktop (`r t_test_effect(abs_error ~ learning.condition, only_conditions(point_data_1, c('Treadmill VR', 'Desktop')))`). Overall, these findings suggested that participants learned the building the best as a result of real-world navigation, followed by the immersive condition, with the desktop condition showing the slowest acquisition.
...
Comparing the improvement in walked distance using one-way ANOVA, we see did not find any  significant difference between groups `r apa_print(aov(block34 ~ learning.condition, df_wide_distance))$full_result$learning_condition `. Visual inspection of the data, however, suggested that the desktop condition again showed the least transfer from block 3 to block 4. For visitation errors, there were significant group differences in error rate improvement, `r apa_print(aov(block34 ~ learning.condition, df_wide_errors))$full_result$learning_condition`. Comparing the conditions pairwise, we can asses that there is a significant difference in improvement between real world learnign and desktop (`r t_test_effect(block34 ~ learning.condition, only_conditions(df_wide_errors, c('Real', 'Desktop')))`), as well as Treadmill VR, `r t_test_effect(block34 ~ learning.condition, only_conditions(df_wide_errors, c('Real', 'Treadmill VR')))`, but we find no difference in improvement betweem desktop learning and Treadmill VR learning, `r t_test_effect(block34 ~ learning.condition, only_conditions(df_wide_errors, c('Treadmill VR', 'Desktop')))`.
...
As an additional assay of transfer, we wished to determine whether learning post modality switch was faster in the group that had been exposed to the real-world in the learning blocks compared to the VR groups. Comparing walked distance and visitation errors across between blocks 4 and 6 by running repeated measures ANOVA, we found a significant effect of learning condition on distance,`r apa_print(ez_fit_distance$aov)$full_result$learning_condition` and visitation errors `r apa_print(ez_fit_errors$aov)$full_result$learning_condition`. We also wound significant interaction effect between learning condition and block for both distance `r apa_print(ez_fit_distance$aov)$full_result$learning_condition_exp_block_id` and visitation errors `r apa_print(ez_fit_errors$aov)$full_result$learning_condition_exp_block_id`.

Looking at the improvement in visitations errors from block 4 to block 6 in separate learning conditions using t-tests, we can assess that only Desktop group (`r t_test_effect(errors ~ exp_block_id, only_conditions_blocks(sub_walk_all, c('Desktop'), c(4,6)))`) and Treadmill VR group (`r t_test_effect(errors ~ exp_block_id, only_conditions_blocks(sub_walk_all, c('Treadmill VR'), c(4,6)))`) have improved, whereas group learned in the real world did not improve any more in the transfer phase (`r t_test_effect(errors ~ exp_block_id, only_conditions_blocks(sub_walk_all, c('Real'), c(4,6)))`). 

The story is similar with distance performance, with both Desktop (`r t_test_effect(min_norm_distance ~ exp_block_id, only_conditions_blocks(sub_walk_all, c('Desktop'), c(4,6)))`) and Treadmill VR (`r t_test_effect(min_norm_distance ~ exp_block_id, only_conditions_blocks(sub_walk_all, c('Treadmill VR'), c(4,6)))`) improve from the 4th to the 6th block, but so does the real world learning group (`r t_test_effect(min_norm_distance ~ exp_block_id, only_conditions_blocks(sub_walk_all, c('Real'), c(4,6)))`).

In the last testing block (block 6), we found no differences among the groups in either error rate (`r apa_print(aov(errors ~ learning.condition, only_blocks(sub_walk_all, 6)))$full_result`), nor pointing performance (`r apa_print(aov(abs_error ~ learning.condition, data = sub_sop_all[sub_sop_all$phase == 2,]))$full_result$learning_condition`). However we still see small difference in distance performance (`r apa_print(aov(min_norm_distance ~ learning.condition, only_blocks(sub_walk_all, 6)))$full_result`), with real group (`r m_sd_report(only_conditions_blocks(sub_walk_all, "Real", 6)$min_norm_distance)`) performing better than Treadmill VR group (`r m_sd_report(only_conditions_blocks(sub_walk_all, "Treadmill VR", 6)$min_norm_distance)`)(`r t_test_effect(min_norm_distance ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Treadmill VR', 'Real'), 6))`) and desktop group (`r m_sd_report(only_conditions_blocks(sub_walk_all, "Desktop", 6)$min_norm_distance)`)(`r t_test_effect(min_norm_distance ~ learning.condition, only_conditions_blocks(sub_walk_all, c('Real', 'Desktop'), 6))`).

## Could effects be driven by difficulty with the immersive interface?

To attempt to address this issue, we compared the three different modalities, navigating in the real-world (real-world), navigating on the treadmill with the head-mounted display (immersed), and navigating with a joystick in desktop VR (impoverished) on the first block of the experiment.  Comparing normalized distances, there were no difference between (`r apa_print(aov(min_norm_distance ~ learning.condition, only_blocks(sub_walk_all, 1)))$full_result$learning_condition`), nor in visitation errors (`r apa_print(aov(errors ~ learning.condition, only_blocks(sub_walk_all, 1)))$full_result$learning_condition`).  This suggests that participants were readily able to navigate in the different modalities regardless of the interface.  We did, however, find a significant difference in normalized time (`r apa_print(aov(min_norm_time ~ learning.condition, only_blocks(sub_walk_all, 1)))$full_result$learning_condition`). This effect appeared to be driven largely by the fact that treadmill VR participants took to get used to the interface and therefore having longer trial times at the start.  
...
```
A total of `r nrow(overview)` undergraduate students at UC Davis particiapted in the second experiment. `r sum(finished$no)` participants didn't finish due to motion sickness. Analyses were then conducted on a final set of `r sum(overview$is_ok == "yes", na.rm=T)` (14 female) participants(`r m_sd_report(all_participant[all_participant$is_ok == "yes",]$age)`).
...
First we assessed difference between the two learning conditions in the last block performance usint t-tests. The conditions did not differ in the walking performance (`r t_test_effect(min_norm_distance ~ learning.condition, only_blocks(sub_walk_all_2, c(6)))`) nor visiting errors (`r t_test_effect(errors ~ learning.condition, only_blocks(sub_walk_all_2, c(6)))`), therefore showing that both treadmill VR and real world learning leads to the same performance in the VR.


## Comparing to previous conditions
Running a one-way ANOVA togeher together with other conditions from the first experiment (real to real, Vr - real and desktop to real), shows a significant effect of condition on final walking performance (`r apa_print(aov(min_norm_distance ~ condition, only_blocks(walk_all, c(6))))$full_result`).

```{r}
kable(tukey_report_table(TukeyHSD(aov(min_norm_distance ~ condition, only_blocks(walk_all, c(6))))))

kable(tukey_report_table(TukeyHSD(aov(errors ~ condition, only_blocks(walk_all, c(6))))))
```

Looking at the final walking performance in the Table XXX, we can see that the differences between conditions are truly negligible. Interestingly, we found no significant difference between the conditions when we analysed absolute distance, rather than normalised (`r apa_print(aov(distance ~ condition, only_blocks(walk_all, c(6))))$full_result`). This means that all aprticipants walked the same distance in the end on average, regardless of them being tested on a treadmill or in the real building. We also found no difference between any of the conditions in their final pointing performance, `r apa_print(aov(abs_error ~ condition, sop_all[sop_all$phase==2,]))$full_result`.
...
Assessing the effects of trasfering from the real world to the Treadmill VR, we ran 2 (Learning condition: real vs treadmill VR) x 2 (block 3 vs blcok 4) ANOVA on data for distance walked and visitation errors. There was no effect of block `r apa_print(ez_fit_distance$aov)$full_result$exp_block_id` nor learning condition (`r apa_print(ez_fit_distance$aov)$full_result$learning_condition`) on distance walked and we observed no interaction effects (`r apa_print(ez_fit_distance$aov)$full_result$learning_condition_exp_block_id`). But we found significant interaction effect between block and learning condition in visitation errors, `r apa_print(ez_fit_errors$aov)$full_result$learning_condition_exp_block_id`. 

Pairwise t-tests comparing change in error rate from 3rd to 4th block show, that the group learning on the treadmill VR does show significant difference (`r t_test_effect(errors ~ exp_block_id, only_conditions_blocks(sub_walk_all_2, 'Treadmill VR', 3:4))`) as it effectively didn't change the environment, but the group switching to the VR from the real world demonstrates increase in visitations errors from block 3 (`r m_sd_report(only_conditions_blocks(sub_walk_all_2, "Real", 3)$errors)`) to block 4 (`r m_sd_report(only_conditions_blocks(sub_walk_all_2, "Real", 4)$errors)`), `r t_test_effect(errors ~ exp_block_id, only_conditions_blocks(sub_walk_all_2, 'Real', 3:4))`.
