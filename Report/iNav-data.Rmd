...
walk_all$learning.condition <- recode(walk_all$learning.condition, "real-real" = "Real", "ve-real" = "Desktop", "vr-real" = "Treadmill VR")
sop_all$learning.condition <- sop_all$condition
sop_all$learning.condition <- recode(sop_all$learning.condition, "real-real" = "Real", "ve-real" = "Desktop", "vr-real" = "Treadmill VR")
...
ggplot(conference_walk_all, aes(x = exp_block_id, y = errors, color = factor(learning.condition))) +
  stat_summary(fun.data=mean_cl_normal,position=position_dodge(width=0.1), width = 0.2, geom="errorbar") + 
  stat_summary(fun.y=mean,position=position_identity(), size = 1, geom="line") + 
  ylab("Number of mistakes in office decision") + xlab("Block") + 
  scale_color_manual(values=estimote_pal,"Condition")
...
ggplot(melt(df[,1:4]), aes(x = learning.condition, y = value, fill = variable)) + 
  geom_bar(stat = 'identity', position="dodge") + ylab("Mean sum of errors")
...
df
ggplot(melt(df[,1:4]), aes(x = learning.condition, y = value, fill = variable)) + geom_bar(stat = 'identity', position="dodge") + ylab("Mean normalised distance")
...
## Independent block improvement comparisons between conditions
