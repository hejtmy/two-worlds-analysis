...
2. Transfer will be higher in the real world learning condition than in desktop treadmill condition.
...
- navigational performance change can be measured in different ways
...
### Performance in block 4 is better than in block 1 and 2
...
```
We see taht both real world and treadmill VR learning leads to improvement from block 1 to block 4 in both errors and path travelled. But desktop only in number of errors and marginally in distance improvement.

### Overrall average walking performance in phase 1 vs phase 2
Given that all participants had 3 blocks of trials in the second phase done in the real world, We can assume, if one learning method leads to better transfer than another
```{r, echo=F}
...
(block directly before vs directly after the switch)
```{r, echo=F}

```

### Average walkkig perfomance in block 1 vs 4 
(starting baseline vs switch)
```{r, echo=F}
