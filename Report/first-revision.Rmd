...
summary(av)
apa_print(av)$full_result$learning_condition
...
```{r}
av <- t.test(abs_error ~ learning.condition, data = sub_sop_all_2[sub_sop_all_2$phase == 1, ])
summary(av)
apa_print(av)$full_result
...
```{r}
av <- t.test(abs_error ~ learning.condition, data = sub_sop_all_2[sub_sop_all_2$phase == 2, ])
apa_print(av)$full_result
...
```{r}
av <- aov(abs_error ~ type, data = sop_all[sop_all$phase == 2, ])
summary(av)
apa_print(av)$full_result$learning_condition
