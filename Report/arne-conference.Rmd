...
ggplot(melt(df[,c(1,5)]), aes(x = condition, y = value)) + geom_bar(stat = 'identity', position="dodge") + ylab("Mean sum of errors improvement from block 3 to 4")

ggplot(melt(df[,c(1,6)]), aes(x = condition, y = value)) + geom_bar(stat = 'identity', position="dodge") + ylab("Mean sum of errors improvement from block 1 to 4")
```
...
ggplot(melt(df[,c(1,5)]), aes(x = condition, y = value)) + geom_bar(stat = 'identity', position="dodge") + ylab("Mean normalised distance improvement from block 3 to 4")

ggplot(melt(df[,c(1,6)]), aes(x = condition, y = value)) + geom_bar(stat = 'identity', position="dodge") + ylab("Mean normalised distance improvement from block 1 to 4")
```